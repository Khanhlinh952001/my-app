...
const AddQuestionForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost:4000/Listen83/${number}`);
  ...
  const handleAddQuestion = async (id) => {
    console.log(data);
    try {
      const response = await fetch(`http://localhost:4000/Listen83/${id}`, {
        method: "PUT",
        headers: {
  ...
  const handleClick = async (file, id) => {
    try {
      const imgRef = storageRef(storage, `Listen83/${uuidv4()}`);
